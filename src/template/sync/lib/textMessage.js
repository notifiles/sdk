...
export default async (props) => {
  ...
  for (var i in clouds) {
    ...
    uploadResult = await upload({
      id: cloud.id,
      sourceUrl: buildPaths.textMessage.text,
      filename: buildPaths.textMessage.textFileName,
      auth: cloud.auth,
      settings
    })
    uploadResult = await upload({
      id: cloud.id,
      sourceUrl: buildPaths.textMessage.manifestFileNameBuilt,
      filename: buildPaths.textMessage.manifestFileName,
      auth: cloud.auth,
      settings
    })
  }
